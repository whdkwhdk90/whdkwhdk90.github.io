...
### 자기소개서
#### 본인을 한마디로 소개한다면 어떻게 표현할 수 있는지, 그리고 그 이유는 무엇인가요?(700자이내)
- 
#### 지원 분야를 선택한 이유와 본인이 해당 직무에 적합하다고 판단할 수 있는 근거를 구체적인 경험과 함께 소개해 주세요.(700자이내)
- 
#### 선택할때의기준은 무엇이며, 왜 진학사가 그 기준에 적합한지 서술해 주세요.(700자이내)
- 
...
```
Semi Project의 회원가입 기능과 동일하다. input태그를 벗어나면 blur로 인해서 값이 유효한지 검사한다.
```  
...
```
Semi Project의 로그인 기능과 동일하다. 
Final Project에선 PW찾기 기능을 넣었고 Spring에서의 BCryptPasswordEncoder로 pw가 암호화되어있기 때문에 
  match메소드를 통해 비교하여 동일하면 pw를 바꿀 수 있도록 진행하였다.
```
...
```
마이페이지 수정 버튼 클릭 시 회원가입 시 입력했던 값을 바꿀 수 있도록 구현하였다.
```  
...
```
Youtube Search API를 이용하여 검색한 내용에 대해 동영상 리스트를 가져오도록 하였다.
```  
...
```
FullCalendar API를 사용하였고 입력된 값에 대해서 DB로 저장될 수 있도록 구현하였다.
```   
...
```
질문게시판에서는 글 작성 후 수정, 삭제 기능과 테이블 5개마다 끊어서 보여질 수 있도록 페이징처리를 구현하였다.
``` 
